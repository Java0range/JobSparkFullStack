...
const props = defineProps<{
  employerInfo: UserEmployersModel,
  closeDrawer: () => void,
  updateUserInfo: () => void
}>();
...
const name = ref<string>(props.employerInfo.name);

const salary = ref<number>(props.employerInfo.salary);

const experience = ref<number>(props.employerInfo.work_experience);

const remotely = ref<boolean>(props.employerInfo.remotely);

const description = ref<string>(props.employerInfo.description);

const city = ref<string>(props.employerInfo.city);

const phone_number = ref<string>(props.employerInfo.phone_number);

const email = ref<string>(props.employerInfo.email);

const telegram_username = ref<string>(props.employerInfo.telegram_username);
...
const createEmployer = async () => {
  const json = {
    name: name.value === props.employerInfo.name ? "" : name.value,
    salary: salary.value === props.employerInfo.salary ? -1 : salary.value,
    work_experience: Number(experience.value) === Number(props.employerInfo.work_experience) ? -1 : Number(experience.value),
    remotely: remotely.value === props.employerInfo.remotely ? null : remotely.value,
    description: description.value === props.employerInfo.description ? "" : description.value,
    city: city.value === props.employerInfo.city ? "" : city.value,
    phone_number: phone_number.value === props.employerInfo.phone_number ? "" : phone_number.value,
    email: email.value === props.employerInfo.email ? "" : email.value,
    telegram_username: telegram_username.value === props.employerInfo.telegram_username ? "" : telegram_username.value
  }
  createEmployerDisable.value = true;
  try {
    const data = await EmployersService.updateEmployer(json);
    if (data.data === "ok") {
...
<template>
  ...
  <div class="fixed top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 rounded-3xl bg-white z-20 p-10 max-sm:w-full max-lg:w-3/4">
    ...
    <p class="text-lg font-bold mb-3">Изменение Вакансии Шаг {{ resumeCreateSteps }}:</p>
    ...
    <div class="flex items-center justify-center w-full">
      <button @click="resumeCreateSteps != 3 ? resumeCreateSteps += 1 : createEmployer()" class="min-w-36 max-sm:min-w-12 bg-sky-500 hover:bg-sky-400 text-white font-bold py-2 px-4 rounded-xl transition-all">
        {{ resumeCreateSteps != 3 ? "Дальше" : "Сохранить"}}
      </button>
    </div>
  </div>
</template>