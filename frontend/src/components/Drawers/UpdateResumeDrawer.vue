...
const name = ref<string>(props.resumeInfo.name);

const surname = ref<string>(props.resumeInfo.surname);

const job_name = ref<string>(props.resumeInfo.job_name);

const description = ref<string>(props.resumeInfo.description);

const education = ref<string>(props.resumeInfo.education);

const educational_institution = ref<string>(props.resumeInfo.educational_institution);

const faculty = ref<string>(props.resumeInfo.faculty);

const experience = ref<number>(props.resumeInfo.experience);

const expected_salary = ref<number>(props.resumeInfo.expected_salary);

const city = ref<string>(props.resumeInfo.city);

const phone_number = ref<string>(props.resumeInfo.phone_number);

const email = ref<string>(props.resumeInfo.email);

const telegram_username = ref<string>(props.resumeInfo.telegram_username);
...
const createResume = async () => {
  const json = {
    name: name.value === props.resumeInfo.name ? "" : name.value,
    surname: surname.value === props.resumeInfo.surname ? "" : surname.value,
    job_name: job_name.value === props.resumeInfo.job_name ? "" : job_name.value,
    description: description.value === props.resumeInfo.description ? "" : description.value,
    education: education.value === props.resumeInfo.education ? "" : education.value,
    edu_institution: educational_institution.value === props.resumeInfo.educational_institution ? "" : educational_institution.value,
    faculty: faculty.value === props.resumeInfo.faculty ? "" : faculty.value,
    experience: Number(experience.value) === Number(props.resumeInfo.experience) ? -1 : experience.value,
    expected_salary: expected_salary.value === props.resumeInfo.expected_salary ? -1 : expected_salary.value,
    city: city.value === props.resumeInfo.city ? "" : city.value,
    phone_number: phone_number.value === props.resumeInfo.phone_number ? "" : phone_number.value,
    email: email.value === props.resumeInfo.email ? "" : email.value,
    telegram_username: telegram_username.value === props.resumeInfo.telegram_username ? "" : telegram_username.value
  }
  createResumeDisable.value = true;
  try {
    const data = await ResumeService.updateResume(json);
    if (data.data === "ok") {
...
<template>
  ...
  <div class="fixed top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 rounded-3xl bg-white z-20 p-10 max-sm:w-full max-lg:w-3/4">
    ...
    <p class="text-lg font-bold mb-3">Изменение Резюме Шаг {{ resumeCreateSteps }}:</p>
    ...
    <div class="flex items-center justify-center w-full">
      <button :disabled="createResumeDisable" @click="resumeCreateSteps != 4 ? resumeCreateSteps += 1 : createResume()" class="min-w-36 max-sm:min-w-12 bg-sky-500 hover:bg-sky-400 text-white font-bold py-2 px-4 rounded-xl transition-all">
        {{ resumeCreateSteps != 4 ? "Дальше" : "Сохранить"}}
      </button>
    </div>
  </div>
</template>